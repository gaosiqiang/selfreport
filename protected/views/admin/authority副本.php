...
<div class="page-header header-container">
    <div style="margin-top: 5px;margin-bottom: 10px;line-height: 1;text-align: left;text-shadow: 1px 2px 3px #ddd;color :#428bca;font-size: 1.05em;">
        <span class="glyphicon glyphicon-leaf" aria-hidden="true" style="top: 2px"></span>权限管理
    </div>
    ...
    <fieldset>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    <?php echo CHtml::textField('username',$model->username,array('readonly'=>true,'class'=>'form-control'));?>
                </div>
            </div>
            <div class=form-group>
                <?php echo CHtml::label('权限类型', false, array('class'=>'col-sm-3 control-label'));?>
                <div class="col-sm-6" style="padding-top:7px">
                    <?php echo CHtml::radioButtonList('privtype',$privtype,$privtypes,array('separator'=>'&nbsp;&nbsp;')); ?>
                </div>
            </div>
            <div class=form-group style="display:none;" id="usergroups">
                <?php echo CHtml::label('用户组', false, array('class'=>'col-sm-3 control-label'));?>
                <div class="col-sm-6">
                    <?php echo CHtml::listBox('usergroup',$usergroup,$usergroups,array('style'=>'width:163px;height:100px;','multiple'=>'multiple')); ?>
                </div>
            </div>
            <div class="form-group" style="display:none;" id="privlevels">
                <?php echo CHtml::label('权限级别', false, array('class'=>'col-sm-3 control-label'));?>
                <div class="col-sm-6">
                    <?php echo CHtml::dropDownList('privlevel',$privlevel,$privlevels,array('class'=>'form-control')); ?>
                </div>
            </div>
...
<!--                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#mediaModal">查看</button>-->
...
<!--            </div>-->
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <div class="alert alert-warning" style="margin-bottom:0">
                        <?php echo CHtml::checkBoxList('category', $category, $first_cates, array('checkAll'=>'全选','separator'=>'&nbsp;&nbsp;'));?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                <div class="col-sm-3">
                    <div style="float:left;"><?php echo CHtml::radioButton('city_type',false,array('id'=>'select_area'))?></div>
                    <?php echo CHtml::label('大区', false, array('class'=>'control-label pull-right','style'=>'padding-top:0;'));?>
                </div>
                <div class="col-sm-9">
                    <?php echo CHtml::listBox('area',$area,$area_arr,array('style'=>'width:163px;height:100px;','multiple'=>'multiple')); ?>
                </div>
            </div>
            <div class="form-group">
                <div class="col-sm-3">
                    <div style="float:left;"><?php echo CHtml::radioButton('city_type',false,array('id'=>'select_region'))?></div>
                    <?php echo CHtml::label('区域', false, array('class'=>'control-label pull-right','style'=>'padding-top:0;'));?>
                </div>
                <div class="col-sm-9">
                    <?php echo CHtml::listBox('region',$region,$region_arr,array('style'=>'width:163px;height:150px;','multiple'=>'multiple')); ?>
                </div>
            </div>
            <div class="form-group"><span style="color: red">ps. 大区、区域列表中的事业部均仅针对于网店通业务</span></div>
            <div class="form-group">
                <div class="col-sm-3">
                    <div style="float:left;"><?php echo CHtml::radioButton('city_type',true,array('id'=>'select_city'))?></div>
                    <?php echo CHtml::label('自选城市', false, array('class'=>'control-label pull-right','style'=>'padding-top:0;'));?>
                </div>
                <div class="col-sm-9">
                    <div id='select-cities' class="alert alert-warning" style="margin-bottom:10px;">
                        <label><input type="checkbox" id="allcity" name="allcity" value="allcity" <?php if(in_array('all', $cities)) echo 'checked="checked"'; ?>> 勾选全部</label>
                        <label><input type="checkbox" id="allsite" name="allsite" value="allsite" <?php if(in_array('all', $cities) || in_array('9999', $cities)) echo 'checked="checked"'; ?>> 整站数据</label><br>
                        <div style="margin-top:5px;"><?php echo CHtml::textField('cities','',array('class'=>'typeahead input-small','id'=>'cities', 'placeholder'=>'自选城市')); ?>
                        <span class="help-inline"><?php echo CHtml::button('添加',array('id'=>'add-city','class'=>'btn btn-xs btn-info')); ?></span></div>
                        <?php if(!empty($cities)){
                            foreach ($cities as $k=>$v)
                            {
                                if($v==0 || $v==9999) unset($cities[$k]);
                            }
                        }?>
                        <div>
                            <span id="tmp-city"></span>
                            <input type="hidden" name="selected-ids" id="selected-ids" value="<?php echo implode(',', $cities)?>"/>
                        </div>
                        <div id="selected-city" style="margin-top:10px;">
                        <?php if(!empty($cities)){
                            foreach ($cities as $city){
                        ?>
                            <span class="selected-city" id="city-id-<?php echo $city;?>" onclick="del_city(<?php echo $city;?>)"><code><?php echo City::getCityNameById($city);?></code></span>
                        <?php }}?>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-group">
                <?php echo CHtml::label('自选部', false, array('class'=>'col-sm-3 control-label'));?>
                <div class="col-sm-9">
                    <div class="alert alert-warning" style="margin-bottom:0">
                        <?php echo CHtml::checkBoxList('priv_wdt_only', $priv_wdt_only, $cities_wdt_only, array('checkAll'=>'全选','separator'=>'&nbsp;&nbsp;'));?>
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready(function(){
    ...
    /////////大区与城市权限互斥 START/////////
    <?php if ($area) {?>
        $('#select_area').attr('checked','checked');
        $('#select_city').attr('checked',false);
    <?php }?>
    
    <?php if ($region) {?>
        $('#select_region').attr('checked','checked');
        $('#select_city').attr('checked',false);
    <?php }?>
    ...
    if($('#allcity').attr('checked') == 'checked')  //因为没有权限也不会赋属性，所以此处attr没问题
    {
        $('#cities').attr('disabled',true);
        $('#add-city').attr('disabled',true);
        $('#selected-ids').attr('disabled',true);
